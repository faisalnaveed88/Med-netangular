...
export class RestService {
  ...
  setupDemo() {
    return this.httpClient.post('http://localhost:3001/api/mtbc.med.net.Patient', null, {withCredentials: true}).toPromise();
  }
  ...
  signUp(data) {
    ...
    return this.httpClient.post('http://localhost:3001/api/mtbc.med.net.Patient', collector).toPromise()
      .then(() => {
        const identity = {
      ...
      //var blob = new Blob([JSON.stringify(cardData, null, 1047)], {type : 'application/octet-stream'});
      const file1 = new Blob([cardData], {});
        const file = new File([file1], Math.random().toString(36).substr(2, 9) + '-myCard.card', {type: 'application/octet-stream', lastModified: Date.now()});

        const formData = new FormData();
        formData.append('card', file1);
  ...
  getAllPenguins() {
    return this.httpClient.get('http://localhost:3000/api/mtbc.med.net.Patient', {withCredentials: true}).toPromise();
  }
  ...
  buyPenguin() {
    ...
    return this.httpClient.post('http://localhost:3000/api/mtbc.med.net.Patient', transactionDetails, {withCredentials: true}).toPromise();
  }
}
